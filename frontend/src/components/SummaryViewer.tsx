...
export default function SummaryViewer({ summary }: Readonly<Props>) {
  ...
  const downloadMP4 = () => {
    window.open(`${api.defaults.baseURL}/download-mp4`, '_blank');
  };
    ...
    <section id="summary" className="rounded-2xl border border-slate-200 bg-white/90 p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900/60">
      <div className="mb-3 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={downloadMP4}
            className="flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium text-slate-700 transition hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700"
          >
